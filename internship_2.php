<?php
    ...
    if (isset($_GET['id'])){
        $stmt = $conn->prepare("SELECT internship2_cer FROM master_database WHERE id = ? ");
        $stmt->bind_param("s",$_GET['id']);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($result->num_rows == 1){
            $rows = $result->fetch_assoc();
            $t = str_split($rows['internship2_cer'],strpos($rows['internship2_cer'],"upload"));
    ...
    if (isset($_GET['did'])){
        ...
        $stmt1 = $conn->prepare("SELECT internship2_cer FROM $company_name WHERE id = ? ");
