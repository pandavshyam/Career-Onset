<?php
    ...
    if (isset($_GET['id'])){
        $stmt = $conn->prepare("SELECT internship3_cer FROM master_database WHERE id = ? ");
        $stmt->bind_param("s",$_GET['id']);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($result->num_rows == 1){
            $rows = $result->fetch_assoc();
            $t = str_split($rows['internship3_cer'],strpos($rows['internship3_cer'],"upload"));
    ...
    if (isset($_GET['did'])){
        ...
        $stmt1 = $conn->prepare("SELECT internship3_cer FROM $company_name WHERE id = ? ");
